...
using ZbSkin.Frm;
using ZbSkin.Sample.Controls;
using ZbSkin.Sample.Forms;
...
namespace ZbSkin.Sample
{
    public partial class MainForm : SkinMainForm
    {
        ...
        public MainForm()
        {
            ...
            //初始化数据
            ThreadPool.QueueUserWorkItem((state) => { DataManager.Instance.InitData(); });
        }
        ...
        private void OnMenuItemClick(int menuTag)
        {
            switch (menuTag)
            {
                ...
                case Constants.MenuTag.About:
                    ...
                    _context?.Post((state) => { new FrmAbout().ShowDialog(this); }, null);
                    break;
            }
        }
